import {
  ...
  export default function DetailsFarm(props) {
    ...
    // Now use the `farm` prop correctly
    const [farmName, setFarmName] = useState(() => farm && farm.farmName ? farm.farmName : "");
    const [address, setAddress] = useState(farm && farm.address ? farm.address : "");
    const [socialNetworkLink, setSocialNetworkLink] = useState(farm && farm.socialNetworkLink ? farm.socialNetworkLink : "");
    const [mainPic, setMainPic] = useState(farm && farm.mainPic ? farm.mainPic : null);
    const [consumerNum, setConsumerNum] = useState(farm && farm.consumerNum ? farm.consumerNum : -1);
